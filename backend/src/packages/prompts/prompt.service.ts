...
import { ArticlePromptCompletionConfig } from './libs/enums/article-prompt-completion-config.enum.js';
import {
...
class PromptService implements IService {
  ...
  public async generate(): Promise<GenerateArticlePromptResponseDto> {
    const promptJSON = await this.openAIService.createCompletion({
      prompt: ArticlePromptCompletionConfig.MESSAGE,
      temperature: ArticlePromptCompletionConfig.TEMPERATURE,
    });
